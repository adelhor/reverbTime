...
while True:
    user_option = int(
        input(
            "Press 1 if you want calculate the reverberation time \n"
    ...
    if user_option == 1:
        DbRequirement.show_spaces_in_db()
        type_of_space = input(
            "Write the type of space that you wish to calculate the reverberation time. \n"
        )
        volume = float(input("What is the volume of space? \n"))
        total_absorption = np.array([0] * 6)

        while True:
            DbCoefficient.show_db_coefficient()
            material = input(
                "Write the material of the first surface avaiable in database \n"
            )
            ...
            user_input = int(
                input(
                    "If you finish adding surfaces press 1 \n"
                    "If you want to add the next surface press 2 \n"
                )
            )
            if user_input == 1:
                my_space = Space(volume, total_absorption, type_of_space)
                my_space.calculation()
        ...
        my_material = DbCoefficient(
            material_name, Hz125, Hz250, Hz500, Hz1000, Hz2000, Hz4000
        )
